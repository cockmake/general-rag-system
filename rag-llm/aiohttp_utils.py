...
async def main():
    ...
    try:
        ...
        print(f"API返回: {len(test_documents)} 个文档的排序结果")
        print(f"实际返回: {len(result.get('output', {}).get('results', []))} 个文档\n")
        
        for idx, item in enumerate(result.get("output", {}).get("results", []), 1):
            print(f"排名 {idx}: 原索引={item['index']}, 分数={item['relevance_score']:.4f}")
            if 'document' in item:
                print(f"  文档: {item['document']['text'][:60]}...")
            print("-" * 70)
    ...
    try:
        ...
        print(f"查询: {test_query}\n")
        print(f"API返回: {len(test_documents)} 个文档的排序结果")
        print(f"应用top_n=3后返回: {len(result_topn.get('output', {}).get('results', []))} 个文档\n")
        
        for idx, item in enumerate(result_topn.get("output", {}).get("results", []), 1):
            print(f"排名 {idx}: 原索引={item['index']}, 分数={item['relevance_score']:.4f}")
            if 'document' in item:
                print(f"  文档: {item['document']['text'][:60]}...")
            print("-" * 70)
    ...
    print("\n" + "=" * 70)
    print("测试3: 使用斩杀线（score_threshold=0.5）")
    print("=" * 70)
    try:
        ...
        print(f"查询: {test_query}\n")
        print(f"API返回: {len(test_documents)} 个文档的排序结果")
        print(f"应用斩杀线(>=0.5)后返回: {len(result_threshold.get('output', {}).get('results', []))} 个文档\n")
        
        for idx, item in enumerate(result_threshold.get("output", {}).get("results", []), 1):
            print(f"排名 {idx}: 原索引={item['index']}, 分数={item['relevance_score']:.4f} ✅ (>=0.5)")
            if 'document' in item:
                print(f"  文档: {item['document']['text'][:60]}...")
            print("-" * 70)
        
        if len(result_threshold.get('output', {}).get('results', [])) == 0:
            print("⚠️  所有文档都被斩杀线过滤！可以降低阈值。")

    except Exception as e:
        print(f"测试3失败: {e}")
    
    print("\n" + "=" * 70)
    print("测试4: 组合使用（top_n=3 + score_threshold=0.3）")
    print("=" * 70)
    try:
        ...
        print(f"查询: {test_query}\n")
        print(f"API返回: {len(test_documents)} 个文档的排序结果")
        print(f"先过滤(>=0.3)，再取top_n=3")
        print(f"最终返回: {len(result_combined.get('output', {}).get('results', []))} 个文档\n")
        
        for idx, item in enumerate(result_combined.get("output", {}).get('results', []), 1):
            print(f"排名 {idx}: 原索引={item['index']}, 分数={item['relevance_score']:.4f}")
            if 'document' in item:
                print(f"  文档: {item['document']['text'][:60]}...")
            print("-" * 70)
        ...
        if len(result_with_threshold.get('output', {}).get('results', [])) == 0:
            print("⚠️  所有文档都被斩杀线过滤！可以降低阈值。")
