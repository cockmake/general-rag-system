...
class RetrievalDecision(BaseModel):
    ...
    existing_info: List[str] = Field(
        ...
        description=(
            "当前已检索到的有用信息，需详细记录，因为历史轮次的 chunk 内容会被压缩显示，"
            "后续轮次将主要依赖此字段回顾已知内容。"
            "每条记录应包含：来源文件名、chunkIndex、核心内容摘要（尽量保留关键数值、名称、逻辑结论），"
            "避免使用'找到了一些内容'等空泛描述。"
        )
    )
...
- retrieved_round：标记该 chunk 在第几轮被检索到；上一轮检索的 chunk（retrieved_round = 当前轮次 - 1）显示完整内容，更早的 chunk 仅显示首尾各25%（中间50%以 "......" 代替）以节省 token

⚠️ 重要：当前轮次显示完整内容的 chunk（retrieved_round = 当前轮次 - 1）是最后一次完整显示的机会，下一轮它们将被压缩。必须在本轮输出的 existing_info 中对这些 chunk 的关键内容进行详细总结，否则下一轮将无法再完整回顾。如需查阅更早轮次的内容，请参考 tool_history 中对应轮次的 existing_info 字段。
